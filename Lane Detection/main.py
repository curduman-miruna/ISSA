...
while True:
    ...
    upper_left = (round(new_width * 0.45), round(new_height * 0.75))
    upper_right = (round(new_width * 0.52), round(new_height * 0.75))
    ...
    cv2.imshow("Only trapezoid", trapezoid_frame * 255)
    ...
    cv2.imshow("Top down view", top_down_frame)
    ...
    #8. Apply binary
    _, binary_frame = cv2.threshold(sobel_combined, 55, 255, cv2.THRESH_BINARY)
    cv2.imshow("Binary", binary_frame)

    #9. Get coordinates of the lane
    copy_frame = binary_frame.copy()
    copy_frame[:, :round(new_width * 0.1)] = 0
    copy_frame[:, round(new_width * 0.9):] = 0

    left_half = copy_frame[:, :new_width // 2]
    left_half[:, round(new_width * 0.9):] = 0
    ...
    #Find the points
    left_top_y = 0
    left_top_x = abs(int(round((left_top_y - left_b) // left_a)))
    if abs(left_top_x) >= 10**8 or left_top_x > new_width//2 or left_top_x < 0:
        left_top_x = previous_left_top_x
    else:
        previous_left_top_x = left_top_x

    left_bottom_y = new_height
    left_bottom_x = abs(int(round((left_bottom_y - left_b) // left_a)))
    if abs(left_bottom_x) >= 10**8 or left_bottom_x > new_width//2 or left_bottom_x < 0:
        left_bottom_x = previous_left_bottom_x
    else:
        previous_left_bottom_x = left_bottom_x

    right_top_y = 0
    right_top_x = abs(int(round((right_top_y - right_b) // right_a)))
    if abs(right_top_x) >= 10**8 or right_top_x < new_width//2 or right_top_x > new_width:
        right_top_x = previous_right_top_x
    else:
        previous_right_top_x = right_top_x

    right_bottom_y = new_height
    right_bottom_x = abs(int(round((right_bottom_y - right_b) // right_a)))
    if abs(right_bottom_x) >= 10**8 or right_bottom_x < new_width//2 or right_bottom_x > new_width:
